...
def _mask_id(v, keep: int = 4) -> str:
    """Mask all but the last `keep` characters of an identifier.

    Payslips get forwarded, printed and filed, so a full PAN on every slip is
    a standing identity-theft exposure. The employee already knows their own
    number; nobody downstream of the payslip needs it in full.
    POOPS1604M -> XXXXXX604M
    """
    s = str(v or "").strip()
    if not s or s == "\u2014":
        return "\u2014"
...
def _identity(payslip: dict, user: dict) -> dict:
    ...
    return {
        ...
        # Masked — see _mask_id. HR keeps the full value on the profile.
        "pan": _mask_id(pick(stat.get("pan"), payslip.get("panNumber"))),
