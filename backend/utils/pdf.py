...
def build_payslip_pdf(
    ...
    # Earnings include employer PF / insurance so the payslip's gross and net
    # match the salary list and the stored payroll figures (compute_totals
    # counts these CTC components as gross). NOTE: this makes the payslip net
    # higher than the actual bank take-home, because employer PF is paid into
    # the PF account rather than to the employee — done intentionally per HR to
    # keep the list and the payslip consistent.
    earn_defs = [
        ("BASIC", "basic"),
        ("HRA", "hra"),
        ...
        ("Employer Insurance", "employerInsurance"),
    ]
    earn_lines = [
